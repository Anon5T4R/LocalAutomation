...
//!
//! Por que o LocalAutomation precisa disso: um gatilho armado ("todo dia às 9h",
//! "a cada 30 min", "quando aparecer arquivo na pasta") só serve se o app
//! estiver vivo na hora. Sem bandeja, fechar a janela mata o processo e o
//! agendamento simplesmente não acontece — calado. Aqui a janela some, o app
//! segue na bandeja e o gatilho dispara na hora certa.
//!
...
/// Fechar com gatilho armado **nunca** desarma o gatilho.
///
/// A opção "fechar minimiza pra bandeja" é opt-in e vem desligada — mas com um
/// gatilho armado, sair no X faria o agendamento das 9h não acontecer, sem erro
/// nenhum. Um automatizador que para de automatizar porque a janela fechou é
/// pior que a surpresa de o app continuar vivo: vai pra bandeja.
pub fn decide_close(close_to_tray: bool, armed: bool) -> CloseAction {
    if close_to_tray || armed {
        CloseAction::ToTray
    } else {
        CloseAction::Exit
    }
...
pub fn reconcile(app: &AppHandle) {
    ...
    if let Err(e) = res {
        eprintln!("[localautomation] falha ao reconciliar o autostart (want={want}, so={state:?}): {e}");
    }
}
...
/// Monta a bandeja e o gancho de fechar. `armed` diz se há gatilho armado
/// (closure pra este módulo não conhecer o motor de gatilhos).
///
    ...
    if let Some(win) = app.get_webview_window("main") {
        ...
        win.on_window_event(move |event| {
            if let WindowEvent::CloseRequested { api, .. } = event {
                ...
                if decide_close(to_tray, armed()) == CloseAction::ToTray {
                    api.prevent_close();
                    let _ = w.hide();
                }
            }
...
mod tests {
    ...
    const NOVO: &str = r"C:\Program Files\LocalAutomation\LocalAutomation.exe --hidden";
    const VELHO: &str = r"C:\Users\joao\AppData\Local\LocalAutomation\LocalAutomation.exe --hidden";
    ...
    fn espaco_e_caixa_nao_contam() {
        let outro = r"c:\program files\localautomation\localautomation.exe --hidden  ";
    ...
    #[test]
    fn fechar_com_gatilho_armado_vai_pra_bandeja() {
    ...
    fn settings_ida_e_volta_no_disco() {
        let p = std::env::temp_dir().join("localautomation-tray-test-settings.json");
    ...
    fn settings_corrompido_nao_derruba_o_app() {
        let p = std::env::temp_dir().join("localautomation-tray-test-corrompido.json");
